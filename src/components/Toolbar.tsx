...
export default function Toolbar({ busy, status, device, profile, query, onQuery, onProfile, onStart, onStop, onRefresh, onMarker }: Props) {
  ...
  const profileDescription = profile === "deep"
    ? "USB packets and device logs plus the HTTP(S) proxy. Route the iPhone to the endpoint shown under Evidence sources and install its CA from http://mitm.it to inspect accepted HTTPS traffic. Certificate pinning is not bypassed."
